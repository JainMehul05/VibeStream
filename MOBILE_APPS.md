# VibeStream Mobile — iOS + Android Deep Dive

The definitive reference for the **VibeStream React Native app**. Every
...
<p align="center">
  <img src="images/VibeStream-logo.png" alt="VibeStream" width="160" />
</p>
...
VibeStream Mobile is a cross-platform **React Native (Expo SDK 51)** client
for the VibeStream emotion-to-music recommendation service. From a **single
...
still produces a working build that talks to the live VibeStream backends.
...
- The hero shows a 84 × 84 rounded gradient tile with the `musical-notes`
  icon, the `VibeStream` kicker, the `Welcome back` H1 and a tri-feature row
...
```json
"plugins": [
  ["expo-camera", { "cameraPermission": "VibeStream uses the camera to detect your mood from a photo." }],
  ["expo-av",     { "microphonePermission": "VibeStream uses the microphone to detect your mood from your voice." }]
]
```
...
npx eas-cli env:create EXPO_PUBLIC_API_URL --value https://VibeStream-backend-api.vercel.app
npx eas-cli env:create EXPO_PUBLIC_MODAL_API_URL --value https://YOUR-MODAL-INFERENCE-URL
...
| Platform | Identifier              |
| -------- | ----------------------- |
| iOS      | `com.VibeStream.mobile`    |
| Android  | `com.VibeStream.mobile`    |
...
xcrun simctl io booted screenshot ~/Desktop/VibeStream-shot.png
```
...
# 3. Take a screenshot
adb exec-out screencap -p > ~/Desktop/VibeStream-shot.png
```
...
| Camera tab is empty even after Allow         | Stale permission cache                             | Quit the app, reopen; if it persists clear iOS Settings → VibeStream → Camera.           |
