...
docker tag vibestream-backend:${IMAGE_TAG} \
  ${REGION}-docker.pkg.dev/${PROJECT_ID}/vibestream-production-docker/backend:${IMAGE_TAG}
docker push ${REGION}-docker.pkg.dev/${PROJECT_ID}/vibestream-production-docker/backend:${IMAGE_TAG}

# Build Frontend
cd ../frontend
docker build -t vibestream-frontend:${IMAGE_TAG} .
docker tag vibestream-frontend:${IMAGE_TAG} \
  ${REGION}-docker.pkg.dev/${PROJECT_ID}/vibestream-production-docker/frontend:${IMAGE_TAG}
docker push ${REGION}-docker.pkg.dev/${PROJECT_ID}/vibestream-production-docker/frontend:${IMAGE_TAG}

# Build AI/ML
cd ../ai_ml
docker build -t vibestream-ai-ml:${IMAGE_TAG} .
docker tag vibestream-ai-ml:${IMAGE_TAG} \
  ${REGION}-docker.pkg.dev/${PROJECT_ID}/vibestream-production-docker/ai-ml:${IMAGE_TAG}
docker push ${REGION}-docker.pkg.dev/${PROJECT_ID}/vibestream-production-docker/ai-ml:${IMAGE_TAG}
```
...
# Verify deployments
kubectl get all -n vibestream-production
```
...
# View logs
gcloud logging read \
  "resource.type=k8s_container AND resource.labels.cluster_name=vibestream-production-gke" \
  --limit 50 \
  --format json
...
gcloud firestore databases update \
  --database=vibestream-production-firestore \
...
kubectl rollout restart deployment -n vibestream-production
```
...
# View HPA status
kubectl get hpa -n vibestream-production
```
...
```bash
# Update node pool
gcloud container clusters update vibestream-production-gke \
  --enable-autoscaling \
  --min-nodes=2 \
  --max-nodes=15 \
  --region=us-central1
...
kubectl logs <pod-name> -n vibestream-production
```
...
gcloud artifacts repositories describe vibestream-production-docker \
