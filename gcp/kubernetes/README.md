...
gcloud compute addresses create VibeStream-static-ip --global

# 3. (Optional) Cloud Armor security policy
gcloud compute security-policies create VibeStream-armor \
  --description "VibeStream WAF"
gcloud compute security-policies rules create 1000 \
  --security-policy VibeStream-armor \
  --src-ip-ranges "*" \
  --action throttle \
...
* **Container image registry** — push to Artifact Registry
  `${REGION}-docker.pkg.dev/${PROJECT}/VibeStream`.
