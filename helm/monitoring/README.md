...
Promtail, and Tempo, then layers VibeStream-specific extras (dashboards,
...
├── dashboards/                 VibeStream Grafana dashboards (overview, inference, postgres)
└── templates/
    ├── _helpers.tpl
    ├── dashboards-configmap.yaml   wraps the dashboards/ JSON into one ConfigMap
    ├── prometheusrule.yaml         VibeStream SLO alerts (5xx, p95, cold starts)
...
Plus the VibeStream-specific resources installed by the chart's own templates:
* `PrometheusRule` with backend / inference / pod-crash SLO alerts
* `ConfigMap` containing the three VibeStream Grafana dashboards
