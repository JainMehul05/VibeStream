# `helm/vibestream-frontend/` — React SPA chart

Installs the VibeStream React SPA on Kubernetes when you can't (or won't) use
...
```bash
helm upgrade --install vibestream-frontend helm/vibestream-frontend \
  -n VibeStream --create-namespace \
  --set image.tag=$(git rev-parse --short HEAD) \
  --set runtimeConfig.REACT_APP_BACKEND_URL=https://api.VibeStream.example.com
```
...
```yaml
runtimeConfig:
  REACT_APP_BACKEND_URL: "https://api.VibeStream.example.com"
  REACT_APP_MODAL_URL:   "https://YOUR-MODAL-INFERENCE-URL"
...
* [`../vibestream-backend/`](../vibestream-backend/) — Django API + blue/green
