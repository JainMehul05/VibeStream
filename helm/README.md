# VibeStream — Helm charts

Helm 3 charts for the self-hosted variant of VibeStream. The canonical
production deploys to **Vercel + Modal**; this directory targets
Kubernetes clusters as an optional alternative.

```
helm/
├── vibestream-backend/    Django backend chart (blue/green-aware, HPA, PDB, ingress, network policy)
├── vibestream-frontend/   React SPA chart (runtime env.js, read-only rootfs, ingress + cert-manager)
└── monitoring/         Umbrella chart wrapping kube-prometheus-stack + Loki + Tempo + Promtail
                        Ships VibeStream dashboards, SLO PrometheusRule, extra ServiceMonitors
```
...
helm dependency update helm/vibestream-backend

# Lint + dry-render
helm lint helm/vibestream-backend
helm template VibeStream helm/vibestream-backend --namespace=VibeStream

# Install / upgrade
helm upgrade --install VibeStream helm/vibestream-backend \
  --namespace=VibeStream --create-namespace \
  --values helm/vibestream-backend/values.yaml \
  --set image.tag=$(git rev-parse --short HEAD)

# Status + history
helm status VibeStream -n VibeStream
helm history VibeStream -n VibeStream

# Roll back
helm rollback VibeStream -n VibeStream
```
...
```bash
helm upgrade --install VibeStream helm/vibestream-backend \
  --namespace vibestream-prod --create-namespace \
  --values helm/vibestream-backend/values.yaml \
  --values helm/vibestream-backend/values-production.yaml \
