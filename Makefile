...
VERSION        ?= $(GIT_SHA)
REGISTRY       ?= ghcr.io/JainMehul05
BACKEND_IMAGE  ?= $(REGISTRY)/vibestream-backend:$(VERSION)
FRONTEND_IMAGE ?= $(REGISTRY)/vibestream-frontend:$(VERSION)
NGINX_IMAGE    ?= $(REGISTRY)/vibestream-nginx:$(VERSION)

# ---- Environment knobs ------------------------------------------------------
ENV            ?= dev
NAMESPACE      ?= vibestream
KUBE_CONTEXT   ?=
TF_BACKEND     ?= local
...
.PHONY: help
help: ## Show this menu
	@printf "$(CYAN)VibeStream Makefile$(RST)  $(DIM)version=$(VERSION) env=$(ENV)$(RST)\n\n"
...
k8s-logs: ## Tail backend logs
	@kubectl $(KCTX) logs -f deploy/VibeStream-backend -n $(NAMESPACE) --tail=200

k8s-rollout: ## Restart deployments to pull new image
	@kubectl $(KCTX) rollout restart deploy -n $(NAMESPACE)
	@kubectl $(KCTX) rollout status deploy/VibeStream-backend -n $(NAMESPACE)

# ---- Helm ------------------------------------------------------------------
.PHONY: helm-lint helm-template helm-install helm-upgrade helm-uninstall
helm-lint: ## helm lint VibeStream-backend chart
	@helm lint $(HELM_DIR)/VibeStream-backend

helm-template: ## Render the chart locally
	@helm template VibeStream $(HELM_DIR)/VibeStream-backend --namespace=$(NAMESPACE)

helm-install: ## helm install/upgrade
	@helm upgrade --install VibeStream $(HELM_DIR)/VibeStream-backend -n $(NAMESPACE) --create-namespace

helm-upgrade: helm-install ## alias

helm-uninstall:
	@helm uninstall VibeStream -n $(NAMESPACE) || true
...
#   * https://vibestream-app.vercel.app                      (marketing / SPA)
#   * https://vibestream-backend-api.vercel.app             (Django REST)
#   * YOUR-MODAL-INFERENCE-URL (ML)
...
modal-stats: ## Show Modal app stats
	@modal app stats vibestream-inference || true
...
argocd-sync: ## Sync the VibeStream application
	@argocd app sync VibeStream-backend

argocd-status: ## Show app health
	@argocd app get VibeStream-backend
