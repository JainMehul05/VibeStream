...
DNS faults to validate that the VibeStream stack survives realistic
failures.
...
metadata:
  ...
  namespace: VibeStream-staging
spec:
  action: pod-kill
  mode: fixed-percent
  value: "30"
  selector:
    namespaces: [VibeStream-staging]
    labelSelectors:
      app.kubernetes.io/name: VibeStream-backend
  scheduler:
    cron: "@every 5m"
```
