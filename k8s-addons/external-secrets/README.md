...
spec:
  ...
  secretStoreRef:
    ...
    kind: SecretStore
  target:
    name: VibeStream-secrets       # the K8s Secret the workload mounts
...
The Helm chart already references `VibeStream-secrets` via `envFrom`, so
...
* External Secrets re-fetches on the `refreshInterval` cadence (default
  ...
  ```bash
  kubectl annotate externalsecret VibeStream-secrets force-sync=$(date +%s) --overwrite
  ```
