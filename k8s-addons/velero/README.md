...
```bash
# 1. Bucket + IAM
aws s3api create-bucket --bucket VibeStream-velero-backups --region us-east-1
...
velero install \
  ...
  --bucket VibeStream-velero-backups \
...
velero restore create --from-backup VibeStream-daily-20260523020000

# Restore into a different namespace (cross-cluster migration)
velero restore create \
  --from-backup VibeStream-daily-20260523020000 \
  --namespace-mappings VibeStream:VibeStream-dr
```
