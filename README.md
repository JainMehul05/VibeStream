# **VibeStream - Adaptive Music Recommendation Platform**

With the rise of personalized music streaming services, there is a growing need for systems that can recommend music based on users' emotional states and contextual signals.
Realizing this need, **VibeStream** is being developed by **[Son Nguyen](https://github.com/hoangsonww)** in 2024 to provide adaptive music recommendations based on users' detected emotions and interactions.

<p align="center">
  <a href="https://vibestream-app.vercel.app/" target="_blank" rel="noopener noreferrer">
    <img src="images/vibestream-logo.png" alt="VibeStream Logo" width="50%" style="border-radius: 12px">
  </a>
</p>

The **VibeStream** project is an integrated adaptive music recommendation platform that combines frontend, backend, AI/ML models, and data analytics to provide personalized music recommendations based on user emotions and contextual signals. The application analyzes **text, speech, or facial expressions** and suggests music that aligns with the detected emotions, continuously learning from user feedback.

Supporting both desktop and mobile platforms, **VibeStream** offers a seamless user experience with real-time emotion detection and adaptive music recommendations. The project leverages **React for the frontend, Django for the backend, and three advanced, self-trained AI/ML models for emotion detection**. **Data analytics** scripts and **Apache Spark and Hadoop** 
...
> Be aware that there may be fake accounts or handles claiming to be me and soliciting investments or personal information. **I am only "Son Nguyen" at [hoangsonww on GitHub](https://github.com/hoangsonww)** and on any other platforms linked here in this repo.
>
...
The VibeStream app is currently live and deployed on Vercel. You can access the live app using the following link: **[https://vibestream-app.vercel.app](https://vibestream-app.vercel.app).**

Feel free to also visit the backend at [VibeStream Backend API](https://vibestream-backend-api.vercel.app/).
...
[![Deployed with Vercel](https://img.shields.io/badge/Deployed%20with-Vercel-green?logo=vercel)](https://vibestream-app.vercel.app/)
[![Django on Vercel](https://img.shields.io/badge/Django-Vercel-000000?logo=vercel&logoColor=white)](https://vibestream-backend-api.vercel.app/)
...
[![Netlify Backup OK](https://img.shields.io/badge/Netlify%20Backup-OK-brightgreen?logo=netlify)](https://vibestream-emotion-music-app.netlify.app/)
[![Docker Container OK](https://img.shields.io/badge/Docker%20Container-OK-brightgreen?logo=docker)](docker-compose.yml)
[![Jenkins Build Status](https://img.shields.io/badge/Jenkins%20Build-Passing-brightgreen?logo=jenkins)](http://vibestream-app.vercel.app/)
[![Kubernetes Deployment OK](https://img.shields.io/badge/Kubernetes%20Deployment-OK-brightgreen?logo=kubernetes)](kubernetes)
[![GitHub Repo Size](https://img.shields.io/github/repo-size/hoangsonww/VibeStream-Emotion-Music-App?logo=github)](https://github.com/hoangsonww/VibeStream-Emotion-Music-App)

<h2 id="-features">🌟 Features</h2>

The VibeStream project aims to provide the following features:
...
VibeStream supports **passwordless sign-in with passkeys** (WebAuthn / FIDO2) layered on top of the existing JWT auth — passwords keep working unchanged.
...
|---|---|---|
| `WEBAUTHN_RP_ID` | `localhost` | your **frontend** domain, e.g. `vibestream-app.vercel.app` (bare host, no scheme) |
| `WEBAUTHN_RP_NAME` | `VibeStream` | shown in the OS passkey sheet |
| `WEBAUTHN_EXPECTED_ORIGINS` | `http://localhost:3000,http://localhost:3001` | comma-separated full origins, e.g. `https://vibestream-app.vercel.app` |
...
Here is the list of technologies used in the VibeStream project:
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/hoangsonww/VibeStream-Emotion-Music-App.git
   ```

2. **Navigate to the AI/ML directory:**
   ```bash
   cd VibeStream-Emotion-Music-App/ai_ml
   ```
...
[`unc-a4/vibestream-app`](https://unc-a4.sentry.io/settings/projects/vibestream-app/)
project.
...
- **Swagger UI**: `https://vibestream-backend-api.vercel.app/swagger/`.
- **Redoc**: `https://vibestream-backend-api.vercel.app/redoc/`.
...
**VibeStream** uses Jest for backend API testing and Jest with React Testing Library for frontend component testing. The tests ensure that the APIs and components are functioning correctly.
...
3. **Expose the services:**
   ```bash
   kubectl expose deployment vibestream-backend --type=LoadBalancer --port=8000
   kubectl expose deployment vibestream-frontend --type=LoadBalancer --port=3000
   ```
...
We have also included Jenkins pipeline script for automating the build and deployment process. You can use Jenkins to automate the CI/CD process for the VibeStream app.
...
- If you have any suggestions, feature requests, or bug reports, feel free to open an issue [here](https://github.com/hoangsonww/VibeStream-Emotion-Music-App/issues).
...
[🔝 Back to Top](#vibestream---adaptive-music-recommendation-platform)
# VibeStream
