...
1. **Configure secrets**:
   ...
   ```bash
   kubectl -n vibestream-production apply -f oracle-cloud/kubernetes/base/secret-template.yaml
   ```
...
3. **Verify**:
   ...
   ```bash
   kubectl -n vibestream-production get pods,svc,ingress
   ```
...
- Inspect ingress load balancer:
  ```bash
  kubectl -n vibestream-production describe ingress moodify-ingress
  ```
