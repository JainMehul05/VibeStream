...
```
   ┌─────────────────────┐
   │  VibeStream-backend    │  (Service, selector: color=<live>)
   └──────────┬──────────┘
              │
   ┌──────────▼──────────────┐         ┌─────────────────────────┐
...
1. Identify the live color: `kubectl get svc VibeStream-backend -o jsonpath='{.spec.selector.color}'`.
...
4. Smoke test against the staged color's preview Service (a sibling
   `VibeStream-backend-green` ClusterIP that always selects `color=green`).
5. `kubectl patch svc VibeStream-backend --type='json' -p='[{"op":"replace","path":"/spec/selector/color","value":"green"}]'`.
