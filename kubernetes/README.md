...
| File | Purpose | Notes |
| --- | --- | --- |
| `frontend-deployment.yaml` | React single-page app deployment | Uses `VibeStream-frontend` image; replace hostPath volume with a persistent volume in production.
...
| `backend-deployment.yaml` | Django/REST backend deployment | Consumes `VibeStream-backend` image and ConfigMap values; update environment variables per environment.
...
| AWS Production | EKS via Terraform modules | `aws eks update-kubeconfig --name VibeStream-production-eks` | See `../aws/README.md` for Terraform and ECR bootstrap. |
| GCP Production | GKE via Terraform modules | `gcloud container clusters get-credentials VibeStream-production-gke --region us-central1` | See `../gcp/README.md` for node pool and Artifact Registry guidance. |
...
- Namespace created (e.g. `kubectl create namespace VibeStream-production`).
- Secrets for credentials already provisioned (see below).

## Quick Start (Local Baseline)
1. **Build images locally**
   ```bash
   docker build -t VibeStream-frontend:latest ../frontend
   docker build -t VibeStream-backend:latest ../backend
   ```
2. **Switch context and namespace**
   ```bash
   kubectl config use-context kind-VibeStream
   kubectl create namespace VibeStream-local
   ```
3. **Apply shared configuration**
   ```bash
   kubectl -n VibeStream-local apply -f configmap.yaml
   ```
4. **Deploy workloads**
   ```bash
   kubectl -n VibeStream-local apply -f backend-deployment.yaml
   kubectl -n VibeStream-local apply -f backend-service.yaml
   kubectl -n VibeStream-local apply -f frontend-deployment.yaml
   kubectl -n VibeStream-local apply -f frontend-service.yaml
   ```
5. **Verify rollout**
   ```bash
   kubectl -n VibeStream-local get pods,svc
   kubectl -n VibeStream-local port-forward svc/frontend-service 3001:3001
   ```
...
- **Images:** Update the `image:` fields to point to the registry promoted by your CI pipeline, for example `123456789012.dkr.ecr.us-east-1.amazonaws.com/VibeStream-frontend:2025.10.07`.
- **Namespace & Labels:** Align namespaces with the staging/production strategy defined in `../DEPLOYMENT.md` (`VibeStream-staging`, `VibeStream-production`).
...
1. **ConfigMap**: Update `configmap.yaml` or create environment-specific overlays.
   ```bash
   kubectl -n VibeStream-production apply -f configmap.yaml
   ```
2. **Secrets**: Store credentials outside source control.
   ```bash
   kubectl -n VibeStream-production create secret generic VibeStream-secrets \
...
- Enable Horizontal Pod Autoscalers once resource requests/limits are set:
  ...
  kubectl autoscale deployment backend-deployment \
    --cpu-percent=70 --min=3 --max=10 -n VibeStream-production
  ```
...
- Inspect deployments: `kubectl -n VibeStream-production get deploy -o wide`
- Watch rollout: `kubectl -n VibeStream-production rollout status deploy/frontend-deployment`
- Tail logs: `kubectl -n VibeStream-production logs deploy/backend-deployment -f`
- Run smoke test job: `kubectl -n VibeStream-production create job --from=cronjob/smoke-tester smoke-on-demand`
- Roll back quickly: `kubectl -n VibeStream-production rollout undo deploy/backend-deployment`
...
Keeping the Kubernetes manifests aligned with these guides ensures VibeStream behaves consistently across clouds, environments, and release cadence.
