# VibeStream Deployment Runbook

Comprehensive deployment procedures, runbooks, and operational guidelines for VibeStream production environments.
...
VibeStream supports two production deployment topologies. Pick the one
that matches your environment — they don't overlap.

| Path                              | When to use                                                                              | Where it lives in this repo                                          |
|-----------------------------------|------------------------------------------------------------------------------------------|----------------------------------------------------------------------|
| **🟢 Vercel + Modal (canonical)** | Default. Zero-ops, scale-to-zero, what the public VibeStream deployment uses.               | `make deploy-prod` (orchestrates `deploy-modal` + `deploy-vercel-*`) |
...
* `REACT_APP_API_URL` → `https://VibeStream-backend-api.vercel.app`
...
* **Passkeys (WebAuthn):** `WEBAUTHN_RP_ID` and `WEBAUTHN_EXPECTED_ORIGINS`
  ...
  `WEBAUTHN_RP_ID=VibeStream-app.vercel.app` and
  `WEBAUTHN_EXPECTED_ORIGINS=https://VibeStream-app.vercel.app`. Optional:
  `WEBAUTHN_RP_NAME` (default `VibeStream`), `WEBAUTHN_CHALLENGE_TTL_SECONDS`
...
```bash
curl -fsS https://VibeStream-backend-api.vercel.app/users/health/        # Django
curl -fsS https://YOUR-MODAL-INFERENCE-URL/health  # Modal
make perf-smoke API_URL=https://VibeStream-backend-api.vercel.app        # k6 smoke
```
...
helm upgrade --install VibeStream-backend  helm/VibeStream-backend  -n VibeStream --create-namespace
helm upgrade --install VibeStream-frontend helm/VibeStream-frontend -n VibeStream
...
VibeStream employs a production-ready, cloud-native deployment architecture with multiple deployment strategies and comprehensive automation:
...
VibeStream supports three deployment strategies:
...
VibeStream uses **Blue-Green Deployment** as the primary strategy with **Canary Releases** available for high-risk changes.
...
export NAMESPACE=VibeStream-staging
...
- [ ] SSL certificates valid (>30 days) — run
      `nginx/scripts/healthcheck.sh https://VibeStream.example.com`
...
kubectl get namespace VibeStream-production

# Verify current environment
kubectl get service backend-service -n VibeStream-production \
...
ACTIVE_ENV=$(kubectl get service backend-service -n VibeStream-production \
    -o jsonpath='{.spec.selector.environment}')
...
export IMAGE_TAG=v1.2.3
export IMAGE_REGISTRY=your-registry.azurecr.io
export NAMESPACE=VibeStream-production
...
```bash
# Watch canary pods
watch -n 5 'kubectl get pods -n VibeStream-production -l version=canary'
...
**1. Via Jenkins UI:**
- Navigate to VibeStream pipeline
...
curl -X POST "https://jenkins.VibeStream.com/job/VibeStream-pipeline/buildWithParameters" \
...
kubectl get deployments -n VibeStream-production
kubectl get pods -n VibeStream-production -o wide

# 2. Check resource utilization
kubectl top nodes
kubectl top pods -n VibeStream-production

# 3. Create backup
./scripts/backup.sh production

# 4. Update status page
curl -X POST https://status.VibeStream.com/api/incidents \
...
kubectl rollout status deployment/backend-deployment -n VibeStream-staging
kubectl rollout status deployment/frontend-deployment -n VibeStream-staging

# 4. Run smoke tests
./scripts/smoke-tests.sh staging

# 5. Verify staging
curl -f https://staging.VibeStream.com/health || exit 1
```
...
watch -n 5 'kubectl get pods -n VibeStream-production | grep canary'

# 3. Check metrics
kubectl top pods -n VibeStream-production -l version=canary

# 4. Verify error rates
curl https://api.VibeStream.com/metrics/error-rate

# 5. If healthy, proceed to 50%
kubectl patch service backend-service -n VibeStream-production \
...
kubectl rollout status deployment/backend-deployment -n VibeStream-production
kubectl rollout status deployment/frontend-deployment -n VibeStream-production

# 2. Verify all pods running
kubectl get pods -n VibeStream-production

# 3. Run post-deployment tests
./scripts/post-deploy-tests.sh production

# 4. Check error rates
curl https://api.VibeStream.com/metrics | jq '.error_rate'

# 5. Verify key endpoints
curl -f https://api.VibeStream.com/health
curl -f https://api.VibeStream.com/users/login -X POST -d '{}'

# 6. Check database connections
kubectl exec -it deployment/backend-deployment -n VibeStream-production -- \
...
curl -X PATCH https://status.VibeStream.com/api/incidents/$INCIDENT_ID \
  -H "Authorization: Bearer $STATUS_TOKEN" \
...
kubectl rollout history deployment/backend-deployment -n VibeStream-production

# 2. Rollback to previous version
kubectl rollout undo deployment/backend-deployment -n VibeStream-production
kubectl rollout undo deployment/frontend-deployment -n VibeStream-production
...
```bash
# 1. Stop application
kubectl scale deployment --all --replicas=0 -n VibeStream-production

# 2. Restore database from backup
# AWS DocumentDB
aws docdb restore-db-cluster-from-snapshot \
  --db-cluster-identifier VibeStream-production-restored \
  --snapshot-identifier $SNAPSHOT_ID

# GCP Firestore
gcloud firestore import gs://VibeStream-backups/$BACKUP_ID

# 3. Update database connection strings

# 4. Restart application
kubectl scale deployment --all --replicas=3 -n VibeStream-production
```
...
# 4. Quick diagnosis
kubectl logs -f deployment/backend-deployment -n VibeStream-production --tail=100
kubectl describe pod $POD_NAME -n VibeStream-production
...
# Option B: Scale up
kubectl scale deployment backend-deployment --replicas=10 -n VibeStream-production
...
# 7. Monitor recovery
watch -n 5 'kubectl get pods -n VibeStream-production'
...
kubectl top pods -n VibeStream-production --sort-by=cpu

# 2. Check pod logs
kubectl logs $HIGH_CPU_POD -n VibeStream-production --tail=100

# 3. Describe pod
kubectl describe pod $HIGH_CPU_POD -n VibeStream-production

# 4. Scale horizontally
kubectl scale deployment/$DEPLOYMENT --replicas=5 -n VibeStream-production

# 5. Monitor
watch -n 5 'kubectl top pods -n VibeStream-production'

# 6. If persistent, restart pod
kubectl delete pod $HIGH_CPU_POD -n VibeStream-production
```
...
kubectl top pods -n VibeStream-production --sort-by=memory

# 2. Check for memory leaks
kubectl exec -it $POD_NAME -n VibeStream-production -- \
...
kubectl delete pod $HIGH_MEMORY_POD -n VibeStream-production
```
...
aws docdb describe-db-clusters --db-cluster-identifier VibeStream-production
...
kubectl rollout restart deployment/backend-deployment -n VibeStream-production
```
...
aws elasticache reboot-cache-cluster \
  --cache-cluster-id VibeStream-production-redis \
