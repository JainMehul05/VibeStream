# VibeStream — Terraform IaC

Multi-cloud Terraform stack for the self-hosted variant of VibeStream.
Modules live in `modules/` and per-env wrappers in `environments/`.
The **canonical** VibeStream production runs on **Vercel + Modal** —
...
State is stored in S3 (`VibeStream-terraform-state` bucket) with
DynamoDB-backed locking (`VibeStream-terraform-locks` table). Bootstrap
...
1. Create `environments/<env>/` with a `main.tf` that wires this root
   module via a single `module "VibeStream" { source = "../../" … }` block.
