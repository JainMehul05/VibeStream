...
data "aws_iam_policy_document" "external_secrets_assume" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:vibestream:external-secrets-sa"]
    }
    condition {
      test     = "StringEquals"
...
data "aws_iam_policy_document" "external_secrets_policy" {
  statement {
    ...
    resources = [
      "arn:aws:secretsmanager:${var.aws_region}:*:secret:vibestream/*",
    ]
  }
}
...
# --- 2. vibestream-backend (S3 + Secrets read) ---------------------------------
data "aws_iam_policy_document" "backend_assume" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "${local.oidc_subject}:sub"
      values   = ["system:serviceaccount:vibestream:vibestream-backend-sa"]
    }
  }
}
...
output "backend_role_arn" {
  description = "Annotate vibestream-backend-sa with this ARN"
