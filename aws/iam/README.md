# `aws/iam/` — IRSA roles for vibestream on EKS
...
| --- | --- |
| `irsa-examples.tf` | Three example IRSA roles: external-secrets-operator, vibestream-backend, cluster-autoscaler |
...
kubectl -n vibestream annotate sa external-secrets-sa eks.amazonaws.com/role-arn=${EXT}

BE=$(terraform output -raw backend_role_arn)
kubectl -n vibestream annotate sa vibestream-backend-sa  eks.amazonaws.com/role-arn=${BE}
