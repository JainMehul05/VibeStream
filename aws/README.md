# VibeStream AWS Deployment Guide

Complete production-ready deployment guide for VibeStream on Amazon Web Services (AWS).

> ⚠️ **Optional self-host path.** The canonical VibeStream production runs
...
The VibeStream application is deployed on AWS using the following services:
...
db_master_username = "VibeStream_admin"
...
# Monitoring
alert_email = "devops@vibestream.example.com"

# Domain (if using custom domain)
enable_custom_domain = true
domain_name          = "vibestream.example.com"
...
aws eks update-kubeconfig \
  --region us-east-1 \
  --name vibestream-production-eks
...
kubectl create secret generic vibestream-secrets \
  --from-literal=MONGODB_URI="mongodb://username:password@docdb-endpoint:27017/VibeStream?ssl=true" \
...
# Build Backend
cd ../../backend
docker build -t vibestream-backend:${IMAGE_TAG} .
docker tag vibestream-backend:${IMAGE_TAG} ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/vibestream-production-backend:${IMAGE_TAG}
docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/vibestream-production-backend:${IMAGE_TAG}

# Build Frontend
cd ../frontend
docker build -t vibestream-frontend:${IMAGE_TAG} .
docker tag vibestream-frontend:${IMAGE_TAG} ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/vibestream-production-frontend:${IMAGE_TAG}
docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/vibestream-production-frontend:${IMAGE_TAG}

# Build AI/ML
cd ../ai_ml
docker build -t vibestream-ai-ml:${IMAGE_TAG} .
docker tag vibestream-ai-ml:${IMAGE_TAG} ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/vibestream-production-ai-ml:${IMAGE_TAG}
docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/vibestream-production-ai-ml:${IMAGE_TAG}
```
...
kubectl get services -n vibestream-production
```
...
```bash
# Check pod status
kubectl get pods -n vibestream-production -w

# Check logs
kubectl logs -f deployment/backend-deployment -n vibestream-production

# Check services
kubectl get svc -n vibestream-production

# Get load balancer URL
kubectl get ingress -n vibestream-production
```
...
aws secretsmanager create-secret \
  --name VibeStream/production/db-password \
  --secret-string "your-secure-password"

# Retrieve secret
aws secretsmanager get-secret-value \
  --secret-id VibeStream/production/db-password \
  --query SecretString \
  --output text
```
...
aws acm request-certificate \
  --domain-name vibestream.example.com \
  --subject-alternative-names www.vibestream.example.com api.vibestream.example.com \
...
aws cloudwatch get-dashboard \
  --dashboard-name vibestream-production
```
...
aws logs tail /aws/VibeStream/production/backend --follow

# Query logs
aws logs filter-log-events \
  --log-group-name /aws/VibeStream/production/backend \
...
aws eks update-nodegroup-config \
  --cluster-name vibestream-production-eks \
...
```bash
kubectl describe pod <pod-name> -n vibestream-production
