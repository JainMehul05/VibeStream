...
| File                       | Purpose                                                       |
| -------------------------- | ------------------------------------------------------------- |
| `namespaces.yaml`          | `VibeStream`, `VibeStream-staging`, `VibeStream-canary` with PSA labels |
...
| ServiceAccount           | IRSA: `eks.amazonaws.com/role-arn: arn:aws:iam::…:role/VibeStream-backend` |
| StorageClass             | `ebs-sc-gp3` (created by Terraform via the EBS CSI add-on)      |
| Image registry           | `<account>.dkr.ecr.<region>.amazonaws.com/VibeStream-backend`      |
