# `aws/cloudwatch/` — VibeStream CloudWatch dashboard + alarms
...
| Resource | Purpose |
| --- | --- |
| `aws_cloudwatch_dashboard.VibeStream` | Single pane (EKS / RDS / Redis + recent backend 5xx log query) |
| `aws_cloudwatch_metric_alarm.eks_node_cpu_high` | Node CPU > 80% for 15m |
| `aws_cloudwatch_metric_alarm.eks_pod_restart_spike` | > 5 restarts in 5m inside `VibeStream` ns |
