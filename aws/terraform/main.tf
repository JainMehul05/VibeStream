# VibeStream AWS Infrastructure - Main Terraform Configuration
...
# Local Variables
locals {
  name_prefix = "VibeStream-${var.environment}"
...
resource "aws_cloudwatch_log_group" "application_logs" {
  ...
  name              = "/aws/VibeStream/${var.environment}/${each.key}"
...
module "cloudfront" {
  ...
  origin = {
    ...
    origin_id   = "VibeStream-alb"
  }

  default_cache_behavior = {
    allowed_methods        = ["GET", "HEAD", "OPTIONS", "PUT", "POST", "PATCH", "DELETE"]
    cached_methods         = ["GET", "HEAD", "OPTIONS"]
    target_origin_id       = "VibeStream-alb"
