...
class TestRegister:
    ...
    def test_missing_fields_rejected(self, api_client):
        resp = api_client.post("/api/v1/users/register/", {"username": "x"}, format="json")
...
class TestLogin:
    def test_success_returns_tokens(self, api_client, make_user):
        ...
        resp = api_client.post(
            "/api/v1/users/login/", {"username": "bob", "password": "password123"}, format="json"
        )
    ...
    def test_wrong_password_rejected(self, api_client, make_user):
        make_user(username="bob", password="password123")
        resp = api_client.post(
            "/api/v1/users/login/", {"username": "bob", "password": "nope"}, format="json"
        )
        assert resp.status_code == 401

    def test_unknown_user_rejected(self, api_client):
        resp = api_client.post(
            "/api/v1/users/login/", {"username": "ghost", "password": "password123"}, format="json"
        )
        assert resp.status_code == 401
...
class TestTokenRefresh:
    def test_success(self, api_client, make_user):
        ...
        resp = api_client.post(
            "/api/v1/users/token/refresh/", {"refresh": tokens["refresh"]}, format="json"
        )
        assert resp.status_code == 200
        assert "access" in resp.data
    ...
    def test_access_token_not_accepted_as_refresh(self, api_client, make_user):
        ...
        tokens = issue_tokens(make_user())
        resp = api_client.post(
            "/api/v1/users/token/refresh/", {"refresh": tokens["access"]}, format="json"
        )
        assert resp.status_code == 401

    def test_garbage_rejected(self, api_client):
        resp = api_client.post("/api/v1/users/token/refresh/", {"refresh": "junk"}, format="json")
        assert resp.status_code == 401


class TestValidateToken:
    def test_authenticated_ok(self, auth_client):
        resp = auth_client.get("/api/v1/users/validate_token/")
        assert resp.status_code == 200

    def test_unauthenticated_rejected(self, api_client):
        resp = api_client.get("/api/v1/users/validate_token/")
...
class TestVerifyAndReset:
    def test_verify_match(self, api_client, make_user):
        ...
        resp = api_client.post(
            "/api/v1/users/verify-username-email/",
    ...
    def test_reset_password_then_login(self, api_client, make_user):
        ...
        login = api_client.post(
            "/api/v1/users/login/", {"username": "dave", "password": "brandnewpass1"}, format="json"
        )
