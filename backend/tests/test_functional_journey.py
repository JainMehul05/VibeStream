...
def test_full_user_journey(api_client, make_user):
    ...
    assert api_client.get("/api/v1/users/validate_token/").status_code == 200

    # 4. Registration created the profile.
    profile = api_client.get("/api/v1/users/user/profile/")
    ...
    assert api_client.post(f"/api/v1/users/mood_history/{pid}/", {"mood": "joy"}, format="json").status_code == 201
    assert api_client.get(f"/api/v1/users/mood_history/{pid}/").data["mood_history"] == ["joy"]

    # 6. Listening history round-trips.
    assert (
        api_client.post(
            f"/api/v1/users/listening_history/{pid}/", {"track": "Song A"}, format="json"
        ).status_code
        == 201
    )
    assert api_client.get(f"/api/v1/users/listening_history/{pid}/").data["listening_history"] == ["Song A"]

    # 7. Recommendations round-trip.
    track = {"name": "Song A", "artist": "Artist", "external_url": "https://example/x"}
    assert (
        api_client.post(
            f"/api/v1/users/recommendations/{pid}/", {"recommendations": [track]}, format="json"
        ).status_code
        == 201
    )
    assert len(api_client.get(f"/api/v1/users/recommendations/{pid}/").data["recommendations"]) == 1

    # 8. The refresh token yields a fresh access token that still works.
    refreshed = api_client.post("/api/v1/users/token/refresh/", {"refresh": refresh}, format="json")
    assert refreshed.status_code == 200
    api_client.credentials(HTTP_AUTHORIZATION=f"Bearer {refreshed.data['access']}")
    assert api_client.get("/api/v1/users/user/profile/").status_code == 200
    ...
    assert intruder.get(f"/api/v1/users/mood_history/{pid}/").status_code == 403

    # 10. Unauthenticated access is refused.
    anon = APIClient()
    assert anon.get("/api/v1/users/user/profile/").status_code in (401, 403)

    # 11. Account deletion removes the user and the profile.
    assert api_client.delete("/api/v1/users/user/profile/delete/").status_code == 200
    assert User.objects(username="journey").first() is None

    # 12. The deleted user can no longer authenticate.
    relogin = api_client.post(
        "/api/v1/users/login/", {"username": "journey", "password": "password123"}, format="json"
    )
