...
class TestUserProfile:
    def test_get_profile_authenticated(self, auth_client):
        resp = auth_client.get("/api/v1/users/user/profile/")
    ...
    def test_get_profile_unauthenticated(self, api_client):
        resp = api_client.get("/api/v1/users/user/profile/")
        assert resp.status_code in (401, 403)

    def test_update_profile_changes_email(self, auth_client):
        resp = auth_client.put(
            "/api/v1/users/user/profile/update/", {"email": "updated@example.com"}, format="json"
        )
        assert resp.status_code == 200
    ...
    def test_delete_profile_removes_user_and_profile(self, auth_client):
        resp = auth_client.delete("/api/v1/users/user/profile/delete/")
        assert resp.status_code == 200
