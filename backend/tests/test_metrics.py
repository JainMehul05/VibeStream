...
class TestRecorder:
    ...
    def test_record_one_request(self):
        r = MetricsRecorder()
        r.record("/api/v1/users/login/", "POST", 200, 42.0)
        snap = r.snapshot()
        assert len(snap["endpoints"]) == 1
        ep = snap["endpoints"][0]
        assert ep["endpoint"] == "/api/v1/users/login/"
        assert ep["count"] == 1
    ...
    def test_per_endpoint_isolation(self):
        r = MetricsRecorder()
        r.record("/api/v1/users/login/", "POST", 200, 1)
        r.record("/api/v1/users/register/", "POST", 400, 2)
        eps = {e["endpoint"]: e for e in r.snapshot()["endpoints"]}
        assert eps["/api/v1/users/login/"]["error_count"] == 0
        assert eps["/api/v1/users/register/"]["error_count"] == 1
...
class TestStore:
    def test_insert_writes_one_row(self, fake_store):
        store_module.insert_event(
            endpoint="/api/v1/users/login/", method="POST", status=200,
    ...
    def test_aggregate_groups_by_endpoint(self, fake_store):
        for status, lat in [(200, 100), (200, 150), (500, 800)]:
            store_module.insert_event(
                endpoint="/api/v1/users/login/", method="POST", status=status,
                latency_ms=lat, container_id="c1",
            )
        result = store_module.aggregate_window(window="1h")
        login = next(e for e in result["endpoints"] if e["endpoint"] == "/api/v1/users/login/")
...
class TestMiddlewareIntegration:
    ...
    def test_real_endpoint_is_persisted(self, fake_store):
        ...
        r = client.post("/api/v1/users/register/", {}, format="json")
        assert r.status_code in (400, 401)
        # We persisted at least one row for /users/register/.
        assert any(
            ev["meta"]["endpoint"].startswith("/api/v1/users/register")
    ...
    def test_path_collapses_to_route_template(self, fake_store):
        ...
        client.get("/api/v1/users/recommendations/get/abc123/")
        client.get("/api/v1/users/recommendations/get/xyz789/")
...
class TestMetricsEndpoint:
    ...
    def test_rejects_wrong_token(self, fake_store, admin_settings):
        client = APIClient()
        r = client.get("/api/v1/metrics/", HTTP_AUTHORIZATION="Bearer not-the-admin")
        assert r.status_code == 401

    def test_returns_live_and_persisted(self, fake_store, admin_settings):
        client = APIClient()
        # Generate one persisted event.
        client.post("/api/v1/users/register/", {}, format="json")
        r = client.get(
            "/api/v1/metrics/?window=1h",
    ...
    def test_metrics_endpoint_is_not_self_recorded(self, fake_store, admin_settings):
        client = APIClient()
        client.get("/api/v1/metrics/", HTTP_AUTHORIZATION=f"Bearer {_ADMIN}")
