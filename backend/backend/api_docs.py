"""Centralised drf-yasg schema helpers for VibeStream's OpenAPI docs.
...
* **VibeStreamSchemaGenerator** -- a tiny drf-yasg ``OpenAPISchemaGenerator``
...
REST API for [**VibeStream**](https://vibestream-app.vercel.app/) -- an emotion-driven music recommendation app. The Django service you're looking at handles user accounts, profiles, history, and proxies a subset of ML inference calls. The heavy ML lifting (text / speech / face emotion detection + the Deezer-backed recommender + the recency-weighted personalisation model) lives in a separate [scale-to-zero **Modal**](https://modal.com) service. The two share a JWT signing key, so the same access token you get from `/users/login/` works against Modal too -- web and mobile clients upload audio / images **directly** to Modal to avoid round-tripping multi-megabyte bodies through Vercel.
...
Both are tuned to never bite a real user -- only retry loops and scripted abuse trip them. See the [modal_inference README](https://github.com/JainMehul05/VibeStream/blob/master/modal_inference/README.md#rate-limiting) for the full design.
...
* Frontend: <https://vibestream-app.vercel.app/>
* Source: <https://github.com/JainMehul05/VibeStream>
...
# ---------------------------------------------------------------------------
class VibeStreamSchemaGenerator(OpenAPISchemaGenerator):
