# VibeStream Infrastructure Setup Guide

Complete guide for setting up the production-ready deployment infrastructure for VibeStream.

> [!NOTE]
> 💡 **Heads up — this guide covers the self-host path.** If you're
> running VibeStream on Vercel + Modal (the canonical production path),
...
```
 ...
 AKS/OKE     Postgres      Redis         storage        topics (alarms)  WAF
   │
   ├──────► Helm install (helm/VibeStream-backend/ + helm/VibeStream-frontend/)
   │
...
| **Helm charts** | `helm/VibeStream-backend/` (Django, blue/green) · `helm/VibeStream-frontend/` (React SPA, runtime env.js) · `helm/monitoring/` (umbrella + VibeStream SLO rules + Grafana dashboards) |
...
```bash
git clone https://github.com/JainMehul05/VibeStream
...
# Kubernetes
export CLUSTER_NAME=VibeStream-production
export CLUSTER_REGION=us-east-1

# Application
export NAMESPACE_PRODUCTION=VibeStream-production
export NAMESPACE_STAGING=VibeStream-staging
```
...
az group create --name VibeStream-rg --location eastus

# Create cluster
az aks create \
  --resource-group VibeStream-rg \
  --name $CLUSTER_NAME \
  --node-count 3 \
...
# Get credentials
az aks get-credentials --resource-group VibeStream-rg --name $CLUSTER_NAME
```
...
```bash
# Create ConfigMap
kubectl create configmap VibeStream-config \
...
kubectl create serviceaccount VibeStream-backend-sa -n $NAMESPACE_PRODUCTION
kubectl create serviceaccount VibeStream-frontend-sa -n $NAMESPACE_PRODUCTION
...
2. Enter name: "VibeStream-pipeline"
3. Select "Pipeline"
...
Promtail, and Tempo, pre-wired with the VibeStream dashboards and SLO alerts.
...
The VibeStream folder is auto-populated by the dashboards in
...
# Add to deployment
kubectl patch serviceaccount VibeStream-backend-sa \
