...
import { API_V1_URL, MODAL_API_URL } from "../../config";
...
const ProfilePage = () => {
  ...
  const fetchUserData = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${API_V1_URL}/users/user/profile/`, {
  ...
  const onSaveUsername = async () => {
    ...
    try {
      const res = await axios.put(
        `${API_V1_URL}/users/user/profile/update/`,
        { username: next },
        authHeaders,
      );
  ...
  const onChangePassword = async () => {
    ...
    setBusy(true);
    try {
      await axios.post(`${API_V1_URL}/users/reset-password/`, {
  ...
  const onDeleteAccount = async () => {
    setBusy(true);
    try {
      await axios.delete(`${API_V1_URL}/users/user/profile/delete/`, authHeaders);
  ...
  const onClearList = async (kind) => {
    ...
    try {
      if (kind === "recommendations") {
        await axios.delete(
          `${API_V1_URL}/users/recommendations/${userId}/`,
          authHeaders,
        );
      } else {
        ...
        for (const item of items) {
          try {
            await axios.delete(`${API_V1_URL}/users/${kind}/${userId}/`, {
              ...authHeaders,
              data: { [entryKey]: item },
            });
          } catch {
