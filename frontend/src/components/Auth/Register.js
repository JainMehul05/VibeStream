...
const Register = () => {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API_V1_URL}/users/register/`, {
        username,
        email,
        password,
      });
      if (response.status === 201) {
        ...
        if (isPasskeySupported()) {
          try {
            const loginRes = await axios.post(`${API_V1_URL}/users/login/`, {
              username,
              password,
            });
    ...
    <div style={styles.container}>
      <Paper elevation={6} sx={styles.formContainer}>
        ...
        <Box sx={styles.heroStrip}>
          ...
          <Typography sx={styles.heroBrand}>VibeStream</Typography>
