...
const HomePage = () => {
  ...
  const saveToHistory = async (mood, recommendations) => {
    try {
      ...
      if (mood) {
        await axios.post(
          `${API_V1_URL}/users/mood_history/${userData.id}/`,
          { mood },
          {
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      if (recommendations && recommendations.length > 0) {
        await axios.post(
          `${API_V1_URL}/users/recommendations/${userData.id}/`,
          {
...
const styles = {
  ...
  micPulseRing: {
    ...
    animation: "vibestreamMicPulse 1.4s ease-out infinite",
    "@keyframes vibestreamMicPulse": {
