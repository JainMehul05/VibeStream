...
const DJANGO_TEXT_PATH = "/text_emotion/";
const DJANGO_MUSIC_PATH = "/music_recommendation/";
...
export function detectTextEmotion({ text, token = null }) {
  ...
  if (isAuthenticated()) {
    return _post(`${API_V1_URL}${DJANGO_TEXT_PATH}`, { text }, cfg);
  }
...
export function getRecommendations({
  ...
  const cfg = maybeAuth(token);
  if (isAuthenticated()) {
    return _post(`${API_V1_URL}${DJANGO_MUSIC_PATH}`, payload, cfg);
  }
