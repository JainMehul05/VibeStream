...
export async function registerPasskey({ name, accessToken } = {}) {
  ...
  const begin = await axios.post(
    `${API_V1_URL}/users/passkeys/register/begin/`,
    {},
    authConfig,
  );
  ...
  const complete = await axios.post(
    `${API_V1_URL}/users/passkeys/register/complete/`,
...
export async function loginWithPasskey({ username } = {}) {
  ...
  const begin = await axios.post(
    `${API_V1_URL}/users/passkeys/login/begin/`,
    username ? { username } : {},
  ...
  const complete = await axios.post(
    `${API_V1_URL}/users/passkeys/login/complete/`,
    { flowId, credential },
    { _skipAuthRefresh: true },
  );
...
export async function listPasskeys() {
  const res = await axios.get(`${API_V1_URL}/users/passkeys/`);
  return res.data.passkeys || [];
}

export async function renamePasskey(id, name) {
  const res = await axios.patch(`${API_V1_URL}/users/passkeys/${id}/`, { name });
  return res.data.passkey;
}

export async function deletePasskey(id) {
  await axios.delete(`${API_V1_URL}/users/passkeys/${id}/`);
}
