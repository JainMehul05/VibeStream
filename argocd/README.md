...
└── applications/
    └── vibestream-backend.yaml     Application that points at helm/vibestream-backend
```
...
## Register the VibeStream application

```bash
kubectl apply -f applications/vibestream-backend.yaml
```

This `Application` points at `helm/vibestream-backend` in this repo and
syncs into the `VibeStream` namespace. Argo CD will:
...
Default policy in `applications/vibestream-backend.yaml`:
...
# Watch sync progress
argocd app wait vibestream-backend --timeout 300

# Diff before sync
argocd app diff vibestream-backend
```
